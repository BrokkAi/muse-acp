...
| Muse skills | ACP `available_commands_update` is refreshed from MSP `skill/list` (and `skill/changed`); slash invocations such as `/plan` use native `skill` turn parts when the selector is in that catalog, with host `skillNotFound` errors preserved; other leading-slash text (for example an absolute path) is sent as ordinary text |
