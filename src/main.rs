...
fn handle_acp(host: &Arc<MspHost>, stdout: &StdoutShared, sessions: &Sessions, msg: &J) {
    ...
    match method.as_str() {
        ...
        "session/list" => {
            ...
            let owned: Vec<(String, String, Option<String>)> = sessions
                .lock()
                .unwrap()
                .values()
                .map(|s| (s.msp_sid.clone(), s.cwd.clone(), s.session_name.clone()))
                .collect();
                ...
                Ok(r) => {
                    ...
                    if let Some(J::Arr(items)) = r.get("sessions") {
                        for item in items {
                            ...
                            let owned_entry = owned.iter().find(|(m, _, _)| m == msp_id);
                            let cwd = item
                                .get("workspaceRoot")
                                .and_then(|v| v.as_str())
                                .or_else(|| owned_entry.map(|(_, c, _)| c.as_str()))
                                .unwrap_or("");
                            let mut parts = vec![
                    ...
                    let entries: Vec<String> = owned
                        ...
                        .map(|(m, c, name)| {
                            let title = name
                                ...
                                .unwrap_or_default();
                            format!("{{\"sessionId\":{},\"cwd\":{}{title}}}", esc(m), esc(c))
                        })
                        .collect();
                    acp::send_result(
                        stdout,
